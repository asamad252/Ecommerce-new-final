...
export default async function OrderSuccessPage({
  ...
  if (!user) {
    // If not authenticated or offline mock mode
    return (
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="mt-3 text-[#114C5A]">
              Your order #{orderId} has been successfully recorded.
            </p>

            <div className="mt-8 flex justify-center gap-4">
              <Link
                href="/shop"
                className="inline-flex items-center gap-2 rounded-xl bg-[#FFC801] px-6 py-3 font-bold text-[#172B36] transition hover:bg-[#FF9932]"
              >
                Continue Shopping
                <ArrowRight size={18} />
              </Link>
  ...
  const formattedDate = new Date(order.created_at).toLocaleDateString("en-PK", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
    ...
    <main className="min-h-[70vh] bg-[#F1F6F4] py-12 md:py-16">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-[#D9E8E2] bg-white p-6">
            ...
            <div className="mt-5 space-y-2 text-sm text-[#114C5A]">
              <p className="font-bold text-[#172B36]">{order.shipping_name}</p>
          ...
          {/* Status */}
          <div className="rounded-2xl border border-[#D9E8E2] bg-white p-6">
            <h2 className="text-lg font-black text-[#172B36]">Order status</h2>

            <div className="mt-5 space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#114C5A]">Order status</span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#114C5A]">Payment</span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#114C5A]">Order date</span>

                <span className="text-sm font-bold text-[#172B36]">{formattedDate}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-[#D9E8E2] bg-white p-6">
          <h2 className="text-xl font-black text-[#172B36]">Order summary</h2>
            ...
            {order.order_items?.map((item) => (
              ...
              >
                <div className="min-w-0">
                  <p className="font-bold text-[#172B36]">{item.product_name}</p>

                  <p className="mt-1 text-sm text-[#114C5A]">Qty: {item.quantity}</p>

                  {item.sku && (
                    <p className="mt-1 text-xs text-[#114C5A]/60">SKU: {item.sku}</p>
                  )}
                </div>
          ...
          <div className="mt-6 border-t border-[#D9E8E2] pt-5">
            <div className="space-y-3 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-[#172B36]">
                  Rs. {Number(order.shipping_fee).toLocaleString("en-PK")}
                </span>
              </div>

              {Number(order.discount) > 0 && (
                <div className="flex items-center justify-between">
                  <span className="text-[#114C5A]">Discount</span>

                  <span className="font-bold text-[#114C5A]">
                    - Rs. {Number(order.discount).toLocaleString("en-PK")}
                  </span>
                </div>
              )}

              <div className="flex items-center justify-between border-t border-[#D9E8E2] pt-4">
                <span className="text-lg font-black text-[#172B36]">Total</span>
